...
var uriResolvers = {
    directory: function directory(parentUrl, directory) {
        addDirectoryUriProperty(parentUrl, directory);
    },

    server: function server(parentUrl, server) {
        addServerUriProperty(server);
    }
};
...
exports.attach = function attach(parentUrl) {
	return function resolveAndAttachUris(result) {
        ...
        children.forEach(function(child){
            ...
            resolver && resolver(parentUrl, child);
        });

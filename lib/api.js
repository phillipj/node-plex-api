...
PlexAPI.prototype._request = function _request(options) {
    ...
    var requestHeaders = headers(this, extend({
        ...
        'X-Plex-Token': options.overrideToken || this.accessToken || this.authToken,
    ...
    request(reqOpts, function onResponse(err, response, body) {
        ...
        if (response.statusCode === 403 && !options.overrideToken && self.authToken) {
            options.overrideToken = self.authToken;
            return deferred.resolve(self._request(options));
        }
...
PlexAPI.prototype._credentialsAuthenticator = function _credentialsAuthenticator() {
    var credentials, credentialsAuth, self;
    ...
    if (this.username && this.password) {
        ...
        credentialsAuth.on('authToken', function (authToken) {
            self.authToken = authToken;
        });
        credentialsAuth.on('accessToken', function (accessToken) {

...
PlexAPI.prototype.find = function(relativeUrl) {
	...
	this.query(relativeUrl, function(err, result) {
		...
		var matchedChildren = filterChildrenByCriteria(allChildren, criterias);
...
function filterChildrenByCriteria(children, criterias) {
	var matchedChildren;
	...
	for (var childName in children) {
		...
		matchedChildren = children[childName].filter(function(child) {
			...
			for (var rule in criterias) {
				...
				if (!child.attributes || child.attributes[rule] !== criterias[rule]) {
					isMatch = false;
					break;
				}
			}

...
PlexAPI.prototype.getIdentifier = function getIdentifier() {
    if(!this.options.identifier) {
        this.options.identifier = uuid.v4();
    }
...
PlexAPI.prototype._authenticateAndRetry = function _authenticateAndRetry(url, retryfunc) {
    ...
    // Bit of a kludgey way to generate an Identifier before we send off the options to auth.
    // Right now it doesn't appear that using the same identifier on every request is necessary, but
    // it's probably good practice to do so.
    self.getIdentifier();
